...
def formatting_file(n):
    ...
    modified_human_genes_file = open(pre_mrna_REGION_FILE_PATH+"\/region_group_" + str(n) + ".fasta", "w")

    for seq_record in SeqIO.parse(pre_mrna_SEQ_FILE_PATH+"\group_" + str(n) + ".fasta", "fasta"):
        if GROUP == 'Vertebrate':
