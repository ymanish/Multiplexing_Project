...
def region_header(exon_start, exon_end, uutr_starts, uutr_stops, dutr_starts, dutr_stops):
...
def formatting_file(n):
    ...
    for seq_record in SeqIO.parse(input_file_path+"\group_" + str(n) + ".fasta", "fasta"):
        ...
        if Group == 'Eukaryote':
            ...
            GENE_ID, TRANSCRIPT_ID, FIVE_UTR_START, FIVE_UTR_END, THREE_UTR_START, THREE_UTR_END, EXON_STARTS, EXON_ENDS, TSS, EXON_RANK = genomic_positions_eukaryote(
                cod_strand, header)
        else:
            header = (seq_record.id).split('|')
            print(header[1])
